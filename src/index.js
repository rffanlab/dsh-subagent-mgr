...
export function apply(ctx) {
  ...
  const reload = async () => {
    const nextStore = await readStore(path)
  ...
  const commit = async nextStore => {
    ...
    await writeStore(path, normalized)
    return normalized
  }
  ...
  ctx.effect(() => {
    ...
    mkdir(dir, { recursive: true }).then(() => {
      ...
      const watcher = watch(dir, { persistent: false }, (_event, filename) => {
        if (filename && basename(String(filename)) !== basename(path)) return
        clearTimeout(timer)
      ...
      else ctx.effect(() => () => watcher.close(), 'dsh-subagent-mgr state watcher')
    }).catch(error => console.error(`[dsh-subagent-mgr] cannot watch state directory: ${error?.stack ?? error}`))
    return () => {
      closed = true
      clearTimeout(timer)
    }
  }, 'dsh-subagent-mgr watcher bootstrap')

  serialize(async () => reconcile(await readStore(path))).catch(error => {
    console.error(`[dsh-subagent-mgr] initial load failed: ${error?.stack ?? error}`)
  })
      ...
      handler: invocation => serialize(async () => {
        try {
          ...
          if (op === 'doctor') {
            ...
            const lines = [`state: ${path}`, `registered backends: ${providers.length ? providers.join(', ') : '(none)'}`]
